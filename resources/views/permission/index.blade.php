                ...
                <div class="col-md-12">
                    <div class="panel panel-warning">
                        <div class="panel-heading">
                            <a href="/permission/add"><button class="btn btn-success btn-xs pull-right">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-condensed table-hover">
                            ...
                            @foreach($actions as $action)
                                <?php
                                    $actionClassify = $action->actionClassify;
                                ?>
                                <tr>
                                    <td>{{$action->id}}</td>
                                    <td>{{$action->name}}</td>
                                    <td>{{$action->url}}</td>
                                    <td>{{App\Constants\ActionType::trans($action->type)}}</td>
                                    <td>{{$actionClassify->name}}</td>
                                    <td><a href="">删除</a>/<a href="/permission/edit?id={{$action->id}}">修改</a></td>
                                </tr>
                            @endforeach
                        </table>
                    </div>
