...
class PageUtil{
    ...
    public static function getPage($currentPage,$totalNum,$pageSize,$id,$type){
        ...
        if($totalPage < 8){
            for($i = 1;$i<$totalPage;$i++){
                if($i == $currentPage){
                    ...
                    $html .= "<a href='/index{$id}{$type}{$i}'>{$i}</a>";
                }
            }
        }elseif($totalPage > 8){
            if($currentPage <= 4){
        ...
        $html .= "<a href='/index{$id}{$type}{$totalPage}' class='pagination-last' title='尾页'>尾页</a>";
        $nextPage = $currentPage+1;
        if($currentPage+1 < $totalPage){
            $html .= "<a href='/index{$id}{$type}{$nextPage}' class='pagination-next'>下一页</a>";
        }
        return $html;
