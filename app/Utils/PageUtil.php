...
class PageUtil{
    ...
    public static function getPage($currentPage,$totalNum,$pageSize,$id){
        ...
        $html .= "<a href='/list/{$id}t{$totalPage}' class='pagination-last' title='尾页'>尾页</a>";
        $nextPage = $currentPage+1;
        if($currentPage+1 < $totalPage){
            $html .= "<a href='/list/{$id}t{$nextPage}' class='pagination-next'>下一页</a>";
        }
        return $html;
